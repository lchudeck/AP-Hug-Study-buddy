...
(function(){
  ...
  async function send(){
    const type=document.getElementById('report-type').value,details=document.getElementById('report-details').value.trim(),bot=document.getElementById('report-bot').value,status=document.getElementById('report-status'),button=document.getElementById('report-send');
    ...
    const c=context(); const data=new URLSearchParams({'form-name':'study-buddy-problem','bot-field':bot,'issue-type':type,'details':details,'page-title':c.title,'section':c.activeTab,'page-url':c.url});
