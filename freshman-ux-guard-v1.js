...
(function(){
 ...
 function hasEvidence(){
   try{
     const a=JSON.parse(window.APStudyReliability.storage.getItem('aphgTopicSkillMasteryV1')||'{"topics":{}}');
     if(Object.values(a.topics||{}).some(x=>x&&x.attempts>0))return true;
   }catch(e){}
   try{
     const a=JSON.parse(window.APStudyReliability.storage.getItem('aphgAdaptiveCoachV2')||'{"topics":{}}');
     if(Object.values(a.topics||{}).some(x=>x&&x.attempts>0))return true;
   }catch(e){}
   try{
     const a=JSON.parse(window.APStudyReliability.storage.getItem('aphgPracticeMasteryV4')||'{}');
