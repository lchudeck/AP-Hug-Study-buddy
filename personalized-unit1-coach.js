...
(function(){
  ...
  function load(){try{return JSON.parse(window.APHGSafeStorage.getItem(STORE)||'{"topics":{},"diagnoses":{},"history":[]}')}catch(e){return {topics:{},diagnoses:{},history:[]}}}
  ...
  function recordCheck(topic,correct){
    ...
    try{
      const a=JSON.parse(window.APHGSafeStorage.getItem(ADAPTIVE)||'{"topics":{},"misconceptions":{},"frq":{}}');
      ...
      const d=new Date(),dk=`${d.getFullYear()}-${d.getMonth()+1}-${d.getDate()}`;if(!x.days.includes(dk))x.days.push(dk);x.days=x.days.slice(-12);x.next=Date.now()+(correct?DAY:0);a.topics[topic]=x;window.APHGSafeStorage.setItem(ADAPTIVE,JSON.stringify(a));
    }catch(e){}
  }
