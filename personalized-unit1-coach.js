...
(function(){
  ...
  function recordCheck(topic,correct){
    ...
    try{
      ...
      const d=new Date(),dk=`${d.getFullYear()}-${d.getMonth()+1}-${d.getDate()}`;if(!x.days.includes(dk))x.days.push(dk);x.days=x.days.slice(-12);x.next=Date.now()+(correct?DAY:0);a.topics[topic]=x;(window.APStudyReliability?.storage||localStorage).setItem(ADAPTIVE,JSON.stringify(a));
    }catch(e){}
  }
