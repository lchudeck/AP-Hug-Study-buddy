...
(function(){
  ...
  window.aiResultHtml=function(result){
    ...
    return `<div class="ai-result"><div class="score-circle">${earned}<small>/${total}</small></div><p style="text-align:center;color:#475569"><b>Each lettered part is worth 1 point.</b></p>${result.parts.map(p=>{const uncertain=!p.earned&&p.coachingOnly;return `<div class="part-result ${p.earned?'part-earned':uncertain?'part-review':'part-missing'}"><div class="verdict">${p.earned?'✅':uncertain?'🔎':'❌'} Part ${p.letter} (${p.verb}): <b>${p.earned?'1/1':uncertain?'Needs self-check':'0/1'}</b></div><p>${esc(p.feedback)}</p>${!p.earned?`<div class="part-fix"><b>${uncertain?'What to check':'Why this missed'}:</b> ${esc(p.feedback.replace(/^0\/1 — Point not earned\.\s*/,''))}<br><br><b>Strong rewrite/model:</b><br>${esc(p.model||p.fix||'Use a specific, accurate APHG response that directly answers this part.')}</div>`:''}</div>`;}).join('')}<div class="box-${pct>=75?'good':pct>=50?'yellow':'warn'}"><b>Locally verified: ${earned}/${total}</b><p>${esc(result.overall||'')}</p></div><button class="btn-secondary" style="margin-top:12px;width:100%" onclick="aiResult=null;render()">Rewrite and try again</button></div>`;
  };
