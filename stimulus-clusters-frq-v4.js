...
(function(){
 ...
 function evaluatePart(part,text){const verb=part[1],t=(text||'').trim().toLowerCase(),words=t.split(/\s+/).filter(Boolean).length,keys=part[3].toLowerCase().split(/\s+/).filter(x=>x.length>3),hasData=/\d/.test(t),comparison=/(greater|less|more|fewer|higher|lower|while|compared|difference|profile|population)/.test(t),hit=keys.some(k=>t.includes(k))||hasData||((verb==='Identify'||verb==='Describe')&&comparison),reason=verb==='Explain'||verb==='Evaluate'?/(because|therefore|as a result|leads to|due to|which causes|so that|\bso\b|while|when|since|can |may |results? in|raises?|reduces?|increases?|decreases?|creates?|allows?|prevents?|hides?)/.test(t):true;const min=verb==='Identify'?2:verb==='Describe'?8:10;return {ok:words>=min&&hit&&reason,words,hit,reason};}
