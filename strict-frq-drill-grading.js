...
(function(){
  ...
  function gradeDrill(index,answer){
    ...
    const content=rule.accept.some(g=>groupHit(text,g));const cause=/(because|therefore|this leads to|as a result|due to|which leads|which causes|so that|results? in|causes?|produces?|creates?|allows?|prevents?|raises?|lowers?|increases?|decreases?|reduces?|encourages?|limits?|contributes? to|drives?|affects?)/.test(text);const verb=d[0];let earned=content;
    if(verb==='Explain')earned=content&&cause&&wc>=7;else if(verb==='Describe')earned=content&&wc>=3;else if(verb==='Identify')earned=content;
    let why='';if(!content)why='No point: the geographic content is incorrect, unrelated, or too vague for this prompt.';else if(verb==='Explain'&&!cause)why='No point yet: you identified a relevant idea, but the relationship between the cause and effect is not clear.';else if(verb==='Explain'&&wc<7)why='No point yet: the idea is relevant, but the explanation is too short to show the causal relationship clearly.';else if(verb==='Describe'&&wc<3)why='No point yet: add a clear characteristic, effect, or example rather than only naming a term.';else why='Point earned: the response contains accurate APHG content and completes the task verb.';
