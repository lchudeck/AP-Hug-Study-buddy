...
(function(){
  ...
  const R=()=>{try{return typeof window.__aphgReadinessEvidence==='function'?window.__aphgReadinessEvidence():null}catch(e){return null}};
  const RH=r=>!r?'<div class="box-info"><b>Complete some practice to build readiness evidence.</b></div>':`<div class="box-${r.enough?'good':r.done>=3?'yellow':'info'}" style="margin-top:14px"><b>${r.label}</b><p>Study Buddy does not turn limited practice into a precise “ready” percentage.</p><div style="display:grid;gap:6px">${r.checks.map(c=>`<div><b>${c.done?'✓':'○'} ${c.label}</b> <span style="color:#64748b">(${c.detail})</span></div>`).join('')}</div><p><b>${r.enough?'Broad evidence is present. Keep reviewing weak topics and use realistic full-exam practice.':'Build the unchecked evidence before treating a practice score as a readiness signal.'}</b></p></div>`;

  if(typeof homePage==='function')homePage=function(){
    let s={accuracy:null,attempted:0,weak:null};
    try{if(typeof getMasterySnapshot==='function')s=getMasterySnapshot()||s}catch(e){}
    const a=s.attempted>=10&&Number.isFinite(s.accuracy)?`${Math.round(s.accuracy)}%`:'Building evidence';
    const w=s.weak?`Unit ${s.weak.unit}: ${s.weak.name}`:'Complete a short practice set and Study Buddy will find it.';
    const r=R();
    return `<main class="wrap student-proof-home"><section class="card student-proof-hero"><div class="student-proof-kicker">Start with your problem, not a feature name</div><h2>What do you need help with?</h2><p class="lead">Pick the sentence that sounds most like you. Study Buddy will take you to the right tool.</p><div class="student-proof-grid"><button type="button" class="student-proof-card" data-student-route="test"><b>📚 My test is coming up</b><span>Review one unit, its vocabulary and models, then check yourself with AP-style questions.</span></button><button type="button" class="student-proof-card" data-student-route="learn"><b>🧠 I don’t understand this yet</b><span>Get a short CED-aligned explanation, common mix-ups, and a worked example.</span></button><button type="button" class="student-proof-card" data-student-route="misses"><b>🎯 I keep missing questions</b><span>Practice weak topics and retry the same skill with a different question.</span></button><button type="button" class="student-proof-card" data-student-route="vocab"><b>📖 I know the vocab, but I can’t use it</b><span>Practice terms as geographic concepts, not just definitions.</span></button><button type="button" class="student-proof-card" data-student-route="maps"><b>🗺️ Maps & data confuse me</b><span>Practice maps, models, scale, patterns, and visual evidence.</span></button><button type="button" class="student-proof-card" data-student-route="frq"><b>✍️ I don’t know how to write an FRQ</b><span>Start with one command word and one AP-style sentence.</span></button><button type="button" class="student-proof-card" data-student-route="ap"><b>🏆 Am I ready for the AP exam?</b><span>Use mixed-unit AP-style practice and see what still needs work.</span></button></div><div class="student-proof-unsure"><div><b>Not sure what to choose?</b><span>Tell Study Buddy what unit you’re in and how much time you have.</span></div><button type="button" class="btn-primary" data-student-route="unsure">Tell me what to do next</button></div></section><section class="card"><h2>My Progress</h2><p>This is a quick snapshot, not a grade. Practice updates it on this device.</p><div class="readiness-grid"><div class="readiness-tile"><b>${a}</b><span>recent practice accuracy${s.attempted<10?' (shown after 10 questions)':''}</span></div><div class="readiness-tile"><b>${s.attempted||0}</b><span>questions practiced</span></div><div class="readiness-tile"><b>${w}</b><span>recommended review</span></div></div>${RH(r)}<div class="button-row" style="margin-top:14px"><button type="button" class="btn-primary" data-student-route="misses">Show me what to practice next</button><button type="button" class="btn-secondary" data-student-route="ap">Open AP practice</button></div></section></main>`;
  };
