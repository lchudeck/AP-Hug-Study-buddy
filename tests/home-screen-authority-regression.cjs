...
assert.match(routeLayer,/p==='ap'\)typeof startFullExam==='function'\?startFullExam\(\):G\('simulator'\)/,'AP readiness must reuse the full simulator doorway');
assert.match(app,/function startFullExam\(\)\{ active='simulator'; simPhase='intro'; render\(\); \}/,'the full simulator doorway must reset to its intro');
