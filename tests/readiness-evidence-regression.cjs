...
assert.match(tracker,/x\.d\.length>=2/,'multi-day evidence requirement missing');
assert.match(tracker,/x\.m>=20/,'mixed-question evidence requirement missing');
assert.match(tracker,/x\.s>=10/,'stimulus evidence requirement missing');
assert.match(tracker,/x\.f>=1/,'FRQ evidence requirement missing');
