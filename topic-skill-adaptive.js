(function(){
  ...
  function load(){
    try{return JSON.parse(window.APHGSafeStorage.getItem(STORE)||'{"topics":{},"skills":{}}');}
    catch(e){return {topics:{},skills:{}};}
  }
  function save(s){window.APHGSafeStorage.setItem(STORE,JSON.stringify(s));}
  ...
  function legacyTopicStat(topic){
    try{
      const legacy=JSON.parse(window.APHGSafeStorage.getItem(LEGACY_TOPIC_STORE)||'{}')[topic];
  ...
  function topicEvidence(){
    ...
    try{Object.keys(JSON.parse(window.APHGSafeStorage.getItem(LEGACY_TOPIC_STORE)||'{}')).forEach(t=>topics.add(t));}catch(e){}
