(function(){
  ...
  const TOPIC_LABELS={
    ...
    '6.1':'The Origin and Influences of Urbanization','6.2':'Cities Across the World','6.3':'Cities and Globalization','6.4':'The Size and Distribution of Cities','6.5':'The Internal Structure of Cities','6.6':'Density and Land Use','6.7':'Infrastructure','6.8':'Urban Sustainability','6.9':'Urban Data','6.10':'Challenges of Urban Change','6.11':'Challenges of Urban Sustainability',
  ...
  function load(){
    try{return JSON.parse((window.APStudyReliability?.storage||localStorage).getItem(STORE)||'{"topics":{},"skills":{}}');}
  ...
  function legacyTopicStat(topic){
    try{
      const legacy=JSON.parse((window.APStudyReliability?.storage||localStorage).getItem(LEGACY_TOPIC_STORE)||'{}')[topic];
  ...
  function topicEvidence(){
    ...
    try{Object.keys(JSON.parse((window.APStudyReliability?.storage||localStorage).getItem(LEGACY_TOPIC_STORE)||'{}')).forEach(t=>topics.add(t));}catch(e){}
